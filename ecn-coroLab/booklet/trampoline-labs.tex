...
You have to download Trampoline from GitHub. Open the Terminal. The current directory is the home directory of your account. Type: 

\unixcl{git\blanc{}clone\blanc{}https://github.com/TrampolineRTOS/trampoline.git}

Trampoline will be pulled and you end with a \texttt{trampoline} directory in youy home directory. The first thing to do is to compile the \texttt{goil}, OIL compiler. Type:

\unixcl{cd\blanc{}trampoline/goil/makefile-macosx}

Then:

\unixcl{./build.py}

Once goil compilation is done. Check the generated binary is working:

\unixcl{./goil\blanc{}--version}
...
The command \unixcl{arm-none-eabi-gcc\blanc{}--version} should print (maybe with a different version number):
...
The output should be (maybe with a different version number):
...
\noindent
\begin{framed}
Call the supervisor to set the environment variables up !
\end{framed}
...
--templates=/opt/trampoline/goil/templates\blanc{}lab1.oil}}
