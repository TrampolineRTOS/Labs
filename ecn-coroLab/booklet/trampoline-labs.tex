...
Trampoline is installed in \unixcl{/opt/trampoline}. \texttt{goil} configuration generator is installed in \unixcl{/usr/local/bin}. The toolchain and \texttt{stlink}, the tool used to download binaries on the board, are installed in \unixcl{/opt}. All the usefull paths are set in the \unixcl{.profile} startup file of your account.
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=.7]{nucleo.pdf} 
   \label{fig:example}
\end{figure}
...
\begin{figure}[htbp] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[scale=.7]{board.pdf} 
   \label{pic:board}
\end{figure}
...
When button B4 is pressed, the servo stops its round-trip. If it is pressed again, round-trip continue. When the round-trip is stopped the following functions are available:
...
When button B4 is pressed again, the servo continues its round-trip and respects the minimum and maximum positions that have been set.
\end{ex}
