...
Get the \texttt{prets.dmg} file and open it by double clicking on its icon.
It will open the disk image in the \texttt{/Volumes/PRETS} folder.

This disk image contains the following folders:
\begin{itemize}
    \item \texttt{trampoline}: a folder that contains the source code of Trampoline RTOS and some other tools, including \texttt{goil}, the OIL compiler.
    \item \texttt{gcc-arm-none-eabi-8-2018-q4-major}: a folder that contains the gnu compiler toolchain to cross-compile programs for the lab board.
    \item stlink\ldots \todo{check stlink installation}
\end{itemize}
...
\todo{Check if there is also something to do for stlink}
