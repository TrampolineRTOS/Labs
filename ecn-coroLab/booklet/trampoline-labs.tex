...
\title{PRETS\\Lab booklet\\2018-2019}
\author{Jean-Luc B\'echennec, Mikaël Briday, S\'ebastien Faucou}
...
You will work within a \texttt{dmg} file.
A \texttt{dmg} is a disk image file, \textsl{i.e.} a file that contains a small filesystem.

To build a disk image, open a terminal and type the following command:

\unixcl{hdiutil\blanc{}create\blanc{}-size\blanc{}900m\blanc{}-fs\blanc{}HFS+\blanc{}-volname\blanc{}PRETS\blanc{}\normaltilde/Desktop/prets.dmg}

To mount the disk image, click on the icon that appeared on the desktop.

Now, you have to download Trampoline RTOS and gcc-arm.
...
\begin{itemize}
    \item Trampoline RTOS: \url{https://github.com/TrampolineRTOS/trampoline/archive/master.zip}
    \item gcc-arm (Downloads>MacOSX 64 bit): \url{https://developer.arm.com/tools-and-software/open-source-software/developer-tools/gnu-toolchain/gnu-rm}
\end{itemize}

Open the archive and copy the folders to the disk image.
From now on, it is assumed that:
\begin{itemize}
    \item The folder containing Trampoline RTOS is named \texttt{trampoline}.
    \item The folder containing the cross compiler is named \texttt{gcc-arm-none-eabi8-2018-q4-major}.
\end{itemize}

To interact with the board, \texttt{stlink} is also required but it is already installed on the workstations.

\section{Building goil}

The first step to have a working configuration is to build \texttt{goil}, the OIL compiler.

To do so:
\begin{itemize}
    \item open a terminal (you can do it by pressing \texttt{Command+Space} and then type \texttt{Terminal} and hit return).
    \item change your working directory by typing the following command in the terminal:

\noindent
\unixcl{cd\blanc{}/Volumes/PRETS/trampoline/goil/makefile-macosx}

    \item build \texttt{goil} by typing the following command in the terminal:
...
This is done by setting the {\tt PATH} environment variable in your \texttt{.profile} file.
It can be done with the following commands:

\unixcl{echo "export PATH=/Volumes/PRETS/trampoline/goil/makefile-macosx:\$PATH" >>\blanc{}\normaltilde/.profile}

\unixcl{echo "export PATH=/Volumes/PRETS/gcc-arm-none-eabi-8-2018-q4-major/bin:\$PATH" >>\blanc{}\normaltilde/.profile}

To get the \normaltilde character on french apple keyboard, you have to use Alt+n.

It is assumed that the location of the stlink tools are already in the PATH.
...
The commande \unixcl{arm-none-eabi-gcc\blanc{}--version} should print:
...
Last, check that stlink tools are available with the following command: \unixcl{st-info\blanc{}--version}.
...
lab1 is a very simple application with only 1 task called {\tt task1}. It starts automatically (\texttt{AUTOSTART = TRUE \{ ... \}} in the OIL file) and turns on LED A0. Look at the OIL file and the C source file.
...
--templates=/Volumes/PRETS/trampoline/goil/templates\blanc{}lab1.oil}}

The \texttt{--target} option gives the target system (here we generate the OS level data structures of Trampoline for a \texttt{cortex} core with the \texttt{armv7em} instruction set, a \texttt{stm32f303} micro-controller and the board is the \texttt{coroLabBoard}).
...
The OIL file gives the names of the C++ source files (with \texttt{APP_CPPSRC}) and the name of the executable file (with \texttt{APP_NAME}).
...
To upload the application to the board, check that the board is connected to the computer, then type:
...
\begin{ex}
    The \texttt{updateStateOfB0} function implements a small state machine to convert the state of the button to states and events.
