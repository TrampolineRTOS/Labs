...
\namedtodo[SF]{Remove ref to \texttt{coro\_utils}, add text on gdb, add \texttt{init.gdb}
in code.}{The goal of this lab} is to become familiar with OSEK/VDX applications development process and with Trampoline and to understand how fixed priority scheduling works.
...
lab1 is a very simple application with only 1 task called \texttt{task1}. It starts automatically (\texttt{AUTOSTART = TRUE \{ ... \}} in the OIL file) and turns on LED A0. Look at the OIL file and the C source file.
...
Alongside the C files of the kernel configuration, Goil generates a build script for the application (files \texttt{make.py} and \texttt{build.py}).
