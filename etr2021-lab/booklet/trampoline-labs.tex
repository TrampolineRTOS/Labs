...
\begin{itemize}
  ...
  \item \texttt{arm-none-eabi-gcc toolchain} including among other tools a C/C++
...
Trampoline is installed in \unixcl{/home/etr2021/bin/trampoline}. \texttt{goil} configuration generator is installed in \unixcl{/usr/local/bin}. The toolchain is installed in \unixcl{/home/etr2021/bin/} and \texttt{stlink}, the tool used to download binaries on the board, is installed in \unixcl{/usr/bin}. All the usefull paths are set in the \unixcl{.bashrc} startup file of your account.
...
The Master CORO lab board is built around a NUCLEO-F303K8 breakout board (BB). A breakout board is a minimal board with components that are difficult to use in prototyping,  on a breadboard or in a hobbyist design, mainly because these components are tiny SMD\footnote{Surface Mounted Device}. The NUCLEO-F303K8 BB is built around a ST Microelectronics microcontroller, the STM32F303K8T6, which has an ARM Cortex-M4 computing core. It is a 32 bits micro-controller running at up to 72MHz. It embeds 64kB of flash memory to store the program and the constant data and 16kB of SRAM to store the variables. The NUCLEO-F303K8 BB is presented at figure \ref{fig:303K8BB}.
